...
HandshakeRemoveMemoriesPass::removeMemories(handshake::FuncOp funcOp) {
    ...
    llvm::TypeSwitch<Operation *, void>(&op)
        .Case<handshake::MemoryControllerOp>(
            [&](handshake::MemoryControllerOp memOp) {
              // Count the number of memory controllers
              numMC++;

              // Create a function argument for the done control
              body->addArgument(builder.getNoneType(), memOp.getLoc());
              replacements.push_back(
                  {memOp->getResults().back(), body->getArguments().back()});
              argTypes.push_back(builder.getNoneType());
              argNames.push_back(StringAttr::get(
                  ctx, "memDone" + std::to_string(memDoneIdx++)));

              // Create function results for control signals
              for (auto [idx, bbInputs] :
                   llvm::enumerate(memOp.groupInputsByBB())) {
                if (memOp.bbHasControl(idx)) {
                  Value bbCtrl = bbInputs.front();
                  resTypes.push_back(bbCtrl.getType());
                  resNames.push_back(StringAttr::get(
                      ctx, "memCtrl" + std::to_string(memCtrlIdx++)));
                  retOperands.push_back(bbCtrl);
                }
              }
            })
        .Case<handshake::DynamaticLoadOp>(
            [&](handshake::DynamaticLoadOp loadOp) {
              Value addrIn = loadOp.getAddress();
              Value dataOut = loadOp.getDataResult();
        ...
        .Case<handshake::DynamaticStoreOp>(
            [&](handshake::DynamaticStoreOp storeOp) {
              Value addrIn = storeOp.getAddress();
              Value dataIn = storeOp.getData();
              // Create a function result for the addr in from circuit
              resTypes.push_back(addrIn.getType());
              resNames.push_back(StringAttr::get(
  ...
  llvm::copy(endOp->getOperands().take_back(numMC),
  ...
  for (Operation &op : llvm::make_early_inc_range(funcOp.getOps())) {
    if (isa<handshake::MemoryControllerOp, handshake::DynamaticLoadOp,
